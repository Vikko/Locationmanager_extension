...
public class Locationmanager implements Runnable {
	...
	private static final RhoLogger LOG = RhoLogger.RHO_STRIP_LOG ? new RhoEmptyLogger() : 
		new RhoLogger("Locationmanager");

	public static RubyModule LocationmanagerModule;    
	...
	public void run() {
		  LOG.INFO("$$$ Register Locationmanager Ruby class");
   	  	  // register Ruby Module
          LocationmanagerModule = RubyAPI.defineModule("Locationmanager");        
          // register Ruby method
          LocationmanagerModule.getSingletonClass().defineMethod("native_process_string", new RubyOneArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg0, RubyBlock block) {
				if ( arg0 instanceof RubyString )
				{
					...
					return ObjectFactory.createString(result);
					  ...
					  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Locationmanager.native_process_string: wrong argument type.Should be String");
				}
			}
          });
          LocationmanagerModule.getSingletonClass().defineMethod("calc_summ", new RubyTwoArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg1, RubyValue arg2, RubyBlock block) {
				try {
						...
						if (arg1 instanceof RubyInteger) {
							...
							if (arg1 instanceof RubySymbol) {
								...
								x = rv.toInt();
								  ...
								  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Locationmanager.calc_summ: wrong argument type.Should be Fixnum or Integer");
							}
						}
					}
					...
					e.printStackTrace();
					LOG.ERROR("Locationmanager.calc_summ failed with exception", e);
